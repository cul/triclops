...
Rails.application.config.after_initialize do
  ...
  # NOTE: This will also change the value of Ruby's Dir.tmpdir
  ENV['TMPDIR'] = TRICLOPS[:vips_tmp_directory] if TRICLOPS[:vips_tmp_directory].present?
  ...
  FileUtils.mkdir_p(TRICLOPS[:tmp_directory]) unless File.exist?(TRICLOPS[:tmp_directory])
  ...
  FileUtils.mkdir_p(TRICLOPS[:vips_tmp_directory]) unless File.exist?(TRICLOPS[:vips_tmp_directory])
end
